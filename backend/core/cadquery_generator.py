...
class CadQueryGenerator:
    ...
    def generate(self):
        ...
        for part_name, part_data in sorted_parts:
            ...
            desc_name = part_data.get("description", {}).get("name", "Feature")
            self.add_line(f"# --- {part_name}: {desc_name} ---")
    ...
    def generate_sketch_extrusion(self, part, part_num, result_var, euler, trans):
        ...
        depth_opposite = extrusion.get("extrude_depth_opposite_normal", 0)
        draft_angle = extrusion.get("draft_angle", 0)
        ...
        for face_idx, (face_name, face_data) in enumerate(faces):
            ...
            for loop_idx, (loop_name, loop_data) in enumerate(loops):
                ...
                has_circle = any(t == "circle" for _, t, _ in items)
                if items and not has_circle:
                    self.add_line("wp = wp.close()")

        total_depth = depth_normal + depth_opposite
        if draft_angle and draft_angle != 0:
            self.add_line(f"wp = wp.extrude({total_depth}, taper={draft_angle})")
        else:
            ...
            for loop_idx, (loop_name, loop_data) in enumerate(loops):
                ...
                for idx, (num, item_type, item_data) in enumerate(items):
                    if item_type == "line":
                        ...
                        center = item_data["Center"]
                        radius = item_data["Radius"]
                        self.add_line(f"wp = wp.moveTo({center[0]}, {center[1]}).circle({radius})")
    ...
    def generate_pattern(self, pattern, result_var):
        ...
        if pattern_type == "linear":
            ...
            self.add_line(f"pattern_original = {result_var}")
            self.add_line(f"for i in range(1, {count}):")
            self.indent += 1
            self.add_line(f"offset = ({norm_dir[0]}*{spacing}*i, {norm_dir[1]}*{spacing}*i, {norm_dir[2]}*{spacing}*i)")
            self.add_line(f"{result_var} = {result_var}.union(pattern_original.translate(offset))")
            self.indent -= 1
            ...
            axis_end = [center[0] + axis[0], center[1] + axis[1], center[2] + axis[2]]
            self.add_line(f"pattern_original = {result_var}")
            self.add_line(f"for i in range(1, {count}):")
            self.indent += 1
            self.add_line(f"angle = {angle_step} * i")
            self.add_line(f"{result_var} = {result_var}.union(pattern_original.rotate(({center[0]}, {center[1]}, {center[2]}), ({axis_end[0]}, {axis_end[1]}, {axis_end[2]}), angle))")
            self.indent -= 1
    ...
    def generate_fillet(self, fillet, result_var):
        ...
        edge_selector = fillet.get("edge_selector", "all")
    ...
    def generate_chamfer(self, chamfer, result_var):
        ...
        edge_selector = chamfer.get("edge_selector", "all")
